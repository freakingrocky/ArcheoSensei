...
function buildQuizSummary(attempts: QuizAttempt[]): QuizSessionSummary | null {
  ...
  const totalScore = attempts.reduce(
    (sum, entry) => sum + (Number.isFinite(entry.evaluation.score) ? entry.evaluation.score : 0),
    0
  );
  const correctCount = attempts.filter((entry) => entry.evaluation.correct).length;
  ...
  for (const entry of attempts) {
    ...
    const topicLabel = (() => {
      ...
      if (entry.lecture_key && entry.lecture_key.trim()) return entry.lecture_key.trim();
  ...
  if (!suggestions.size) {
    suggestions.set("general-review", "Review the retrieved notes highlighted in each quiz question.");
  }
      ...
      {quizSummaryVisible && quizSummary && (
        <div className="fixed bottom-6 right-6 z-40 w-full max-w-md rounded-2xl border border-neutral-800 bg-neutral-950/95 p-5 shadow-2xl backdrop-blur">
          <div className="flex items-start gap-3">
            <div>
              <h3 className="text-lg font-semibold text-neutral-100">Quiz session summary</h3>
              <p className="text-sm text-neutral-400">
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div className="rounded-lg border border-neutral-800 bg-neutral-900/70 p-3">
              <div className="text-xs uppercase tracking-wide text-neutral-500">Questions attempted</div>
              <div className="mt-1 text-2xl font-semibold text-neutral-100">{quizSummary.attempted}</div>
            </div>
            <div className="rounded-lg border border-neutral-800 bg-neutral-900/70 p-3">
              <div className="text-xs uppercase tracking-wide text-neutral-500">Average score</div>
